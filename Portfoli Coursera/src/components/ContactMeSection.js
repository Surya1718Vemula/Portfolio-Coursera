...
//       try {
//         await submit('api_endpoint_url', values);
//         resetForm();
//         if(response?.type === "success") {
//           const firstName=values.firstName
//           onOpen(response.type, `Thanks for submission ${firstName}, we will get back to you shortly!`)
...
//                 <Select id="type" name="type" {...formik.getFieldProps('select')}>
...
const LandingSection = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        await submit("/api/endpoint", values); 
        resetForm(); // Reset the form
        if (response?.type === "success") {
          const firstName = values.firstName; 
          onOpen(response.type, `Thanks for your submission ${firstName}, we will get back to you shortly!`);
        }
      } catch (error) {
        console.error("Error submitting form:", error);
